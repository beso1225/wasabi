...
const ATTR_WRITE_THGOUGH: u64 = 1 << 3;
...
pub enum PageAttr {
    ...
    ReadWriteIo = ATTR_PRESENT | ATTR_WRITABLE | ATTR_WRITE_THGOUGH | ATTR_CACHE_DISABLE,
}
...
/// # Safety
/// Anthing can happen if the given selector is invalid.
...
struct InterruptInfo {
    // This struct is placed at the top of the interrupt stack.
    // Should be aligned on 16-byte boundaries to pass the alignment checks done by FXSAVE / FXRSTOR
...
// In IA-32e mod, the RSP is aligned ro a 16-byte boundary
// before pushing the stack fram.
...
macro_rules! interrupt_entrypoint_with_error_code {
    ($index:literal) => {
        global_asm!(concat!(
            ".global interrupt_entrypoint",
            stringify!($index),
...
interrupt_entrypoint_with_error_code!(8);
interrupt_entrypoint_with_error_code!(13);
interrupt_entrypoint_with_error_code!(14);
...
impl Idt {
    pub fn new(segment_selector: u16) -> Self {
        ...
        // SAFETY: This is safe since it loads a valid IDT that is constructed in the code just
        // above.
        unsafe {
            asm!("lidt [rcx]",
            in("rcx") &params
...
impl TaskStateSegment64 {
    ...
    unsafe fn alloc_interrupt_stack() -> u64 {
        ...
        // now, no one except us own the region since it is forgotten by thie allocator ;)
        rsp
    }
    pub fn new() -> Self {
...
impl GdtWrapper {
    pub fn load(&self) {
        ...
        info!("Loading GDT @ {:#018X}", params.base as usize);
        // SAFETY: This is safe since it is loading a valied GDT just constructed in the above.
        unsafe {
            asm!("lgdt [rcx]",
            in("rcx") &params
            )
...
/// # Safety
/// Writing to CR3 can causes any exceptions so it is programmer's responsibility to setup correct
/// page tables.
#[no_mangle]
