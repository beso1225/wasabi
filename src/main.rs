...
#[unsafe(no_mangle)]
fn efi_main(_image_hangle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    //         (vram_byte_size / size_of::<u32>() as u64) as usize,
    //     )
    // };
    // for e in vram {
    //     *e = 0xffffff;
    ...
    fill_rect(&mut vram, 0x0000ff, 128, 128, 128, 128).expect("fill_rect_failed");
    for i in 0..256 {
...
fn draw_line<T: Bitmap>(buf: &mut T, color: u32, x0: i64, y0: i64, x1: i64, y1: i64) -> Result<()> {
    ...
    if dx >= dy {
        ...
        for (rx, ry) in (0..dy).flat_map(|ry| calc_slop_point(dy, dx, ry).map(|rx| (rx, ry))) {
            draw_point(buf, color, x0 + rx * sx, y0 + ry * sy)?;
        }
    }
    Ok(())
