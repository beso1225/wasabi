...
impl FirstFitAllocator {
    ...
    fn add_free_from_descriptor(&self, desc: &EfiMemoryDescriptor) {
        ...
        // Make sure the allocator does not include the addres 0 as a free area.
        if start_addr == 0 {
            start_addr += 4096;
        ...
        // It's okay not to be sorted hte headers at this point
...
mod test {
    ...
    fn malloc_align_random_order() {
        for align in [32, 4096, 8, 4, 16, 2, 1] {
            let mut pointer = [null_mut::<u8>(); 100];
            for e in pointer.iter_mut() {
                *e = ALLOCATOR.alloc_with_options(
                    Layout::from_size_align(1234, align).expect("Failed to create Layout"),
                );
                assert!(*e as usize != 0);
