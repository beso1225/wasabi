...
struct SystemDescriptionTableHeader {
    ...
    // Tavle 5.4: DESCRIPTION_HEADER Fields
    signature: [u8; 4],
    length: u32,
    _unused: [u8; 28],
}
...
impl Xsdt {
    ...
    fn heaer_size(&self) -> usize {
        size_of::<Self>()
    }
    fn num_of_entries(&self) -> usize {
        (self.header.length as usize - self.heaer_size()) / size_of::<*const u8>()
    }
    unsafe fn entry(&self, index: usize) -> *const u8 {
        ((self as *const Self as *const u8).add(self.heaer_size()) as *const *const u8)
            .add(index)
            .read_unaligned()
    }
...
trait AcpiTable {
    ...
    fn new(header: &SystemDescriptionTableHeader) -> &Self::Table {
        ...
        // This is safe as far as phys_addr points to a valid MCFG table and it ablies forever.
        let mcfg: &Self::Table =
...
impl GenericAddress {
    pub fn address_in_memory_space(&self) -> Result<usize> {
        if self.address_space_id == 0 {
            ...
            Err("ACPI Generic Address i not in system memory space")
        }
    }
}
